...
// API Routes
  app.get('/api/health', async (req, res) => {
    try {
      ...
      res.json({ status: 'ok', hasGeminiKey: !!process.env.GEMINI_API_KEY, database: 'connected', timestamp: new Date().toISOString() });
    } catch (error) {
      console.error('[HEALTH CHECK FAILED] DB unreachable:', error);
      res.status(503).json({ status: 'error', hasGeminiKey: !!process.env.GEMINI_API_KEY, database: 'disconnected', timestamp: new Date().toISOString() });
    }
  });
...
- requiresApproval: boolean`;
  ...
  app.post('/api/user/sync', requireAuth, async (req: AuthRequest, res) => {
    try {
      ...
      console.error(`[AUTH SYNC FAILED] endpoint: /api/user/sync, status: 500, error: ${error.message || 'Database unreachable or internal error'}`);
      res.status(500).json({ error: 'Backend database tidak dapat dijangkau. Sync failed.' });
    }
  });
  ...
  app.get('/api/admin/users', requireAuth, async (req: AuthRequest, res) => {
    try {
      ...
      res.status(500).json({ error: 'Backend database tidak dapat dijangkau.' });
    }
  });
